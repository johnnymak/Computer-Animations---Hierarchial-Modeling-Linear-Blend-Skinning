...
//Model matrices (Column Major, fills in column first)
double _matrix[16];
double _matrixI[16];
...
void init()
{
    ...
    std::cout<<"GLSL: "<<glGetString(GL_SHADING_LANGUAGE_VERSION)<<std::endl;*/
	 ...
	 GLfloat lightPos[] = { 20.0f, 20.0f, 50.0f, 0 };
...
int main(int argc, char **argv)
{
	...
	// Creating Vector
	Eigen::Vector4f point(1.0, 2.0, 3.0, 1.0);
	std::cout << point << std::endl;

	Eigen::Vector4f transformed = matrix * point;
	std::cout << transformed << std::endl;

	// Dot product
	// float dot_product = transformed_point.dot(another_point);

	// Finding the norm
	float norm = transformed.norm();

	// Cross Product - Only for 3x vectors
	...
	/* Transformation:
		...
		- Joints have Two Matrices:
			- Matrix4f local: only rotations  (default = identity) / After: Local = local * rotation(Theta)
			- Matrix4f global: includes translation and rotations (default = identity)

		GlobalP2 = GlobalP1 * T(p1) * rot(theta) * T(p2) 
